# OK Ready Chatbot Knowledge

## About OK Ready Australia
OK Ready Australia offers a 24-week modular curriculum called the Antifragile System. It's designed for beginners — no prior experience needed. The system covers 8 Pillars of Readiness that help everyday Australians build genuine antifragility through modern self-sufficiency.

## Current Status
Publishing Mid May 2026. Currently accepting free waitlist signups.

## Pricing — How Much Does the Course Cost?
The OK Ready Antifragile System course is a one-time payment of **$79.99** (Early Bird price, going up to **$99.99** in June). There are no subscriptions or recurring fees. This gives you lifetime access to the full 24-week modular curriculum, all 8 Pillars of Readiness, and all future updates.

**What you get for $79.99:**
- Full 24-week modular curriculum — 8 pillars of antifragility
- AI Agent System — automated workflows & decision tools
- Buying Guide — smart purchasing for resilience gear
- Weekly online sessions — curriculum summary, AI Agent system setup & troubleshooting
- Starter Resource Library (books, templates, checklists)
- Lifetime access — all future updates included
- Afterpay payments available
- 7-day money back guarantee
- 10% of profits donated to local food banks

## The 8 Pillars of Readiness

### Pillar 01 — Mindset & Systems
Emergency preparedness, systems thinking, building a 72-hour kit, decision-making under pressure, emergency communication plan. The foundation everything else builds on.

### Pillar 02 — Digital & Cognitive
AI literacy, offline-first AI workflows, CO-STAR system prompts, digital hygiene, privacy, encrypted communications, offline knowledge tools (Kiwix, OsmAnd). Using AI as a force multiplier without becoming dependent on it.

### Pillar 03 — Economic Resilience
Multiple income streams, right-to-repair skills, low-overhead living, financial optionality, avoiding single points of failure in your finances.

### Pillar 04 — Energy & Infrastructure
Solar readiness, backup power, water security, reducing dependency on centralised utilities. Small steps like power banks, rain barrels, gas cooktops.

### Pillar 05 — Food & Material
Sprouts, microgreens, fermentation, container gardening, food preservation. Turning any space into a productive node.

### Pillar 06 — Physical Skills
First aid, mechanical competence, home security, situational awareness. Baseline human capabilities we've outsourced.

### Pillar 07 — Faith & Future
Hope as a skill, faith in a creator, ritual and grounding practices, intergenerational storytelling, finding purpose in hardship. Inner infrastructure.

### Pillar 08 — Community Networks
Local mapping, ham radio, mutual aid, intergenerational skill sharing, private Telegram group. Resilience is a team sport.

## Coaching

### Deep Guidance — $1,500/month
- Private weekly coaching sessions
- Custom antifragile system design for your household
- AI Agent System setup and optimisation
- Direct access via voice, text, and email
- Privacy & NDA included
- Bring up to 3 family or friends

### Complete Guidance — $6,000/month
- Everything in Deep Guidance
- Personal shopping budget for resilience gear
- Full household antifragile transformation
- On-site visits (where feasible)
- Privacy & NDA included
- Limited — 2 spots per quarter

## Workshop for Teens (School Holidays)
- For Ages 13–17
- Two-week program, Mondays & Fridays, 2 hours per session
- July School Holidays and September School Holidays
- Online via Google Meet, max 8 teens
- $395 per teen
- Our trainers hold NDIS and Working with Children checks
- Covers: Emergency preparedness, AI literacy & AI Agents, Basic first aid, Growing food, Digital hygiene, Community building, Trivium & Quadrivium, Side hustles & marketing basics, Confidence & resilience

## The Antifragile Mindset
The core philosophy: antifragility is not about surviving the storm — it's about building a ship that gets stronger the more it sails through rough water. Three categories: Brittle (broken by shock), Robust (unharmed by shock), Antifragile (strengthened by shock).

### Seven Principles
1. Embrace Small Stressors — managed exposure to stress builds strength
2. Build Redundancy — two kidneys, two lungs, two income streams
3. Use Via Negativa — remove fragility before adding strength
4. Favour Decentralisation — local over centralised
5. Practice Optionality — keep options open
6. Learn Through Skin in the Game — real stakes produce real learning
7. Respect the Nonlinear — small changes produce massive effects

## Waitlist — Free Signup
The OK Ready course is currently in pre-launch. Join the free waitlist to get early access when we publish in May 2026. Free to join. Sign up at the waitlist section on the website. Receive the "Why OK Ready?" free PDF as a gift when you sign up.

## Free PDF Resources
- Why OK Ready? — the case for antifragility with current Australian statistics
- The Antifragile Mindset — full philosophy guide
- More Reading & Resources — books, podcasts, tools, Australian-specific resources
- OK Ready Buying Guide — product recommendations with Australian prices

## Contact / Founder
Brendan O'Keefe, OK Ready Australia. Built from years of testing, failing at, and refining these systems.
